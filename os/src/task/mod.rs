...
impl TaskManager {
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if let None = next_task.task_start_time {
    ...
    fn get_current_task_info(&self) -> TaskInfo {
        ...
        TaskInfo {
            ...
            syscall_times: current.task_syscall_info,
            time: get_time_ms() - current.task_start_time.unwrap(),
        }
    }

    /// current task did a syscall, count it.
    fn syscall_counter(&self, syscall_id: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].task_syscall_info[syscall_id] += 1;
    }
}
